...
Page({
  ...
  data: {
    ...
    items: [
      { name: 'backin', value: '还车险' },
      { name: 'wheelin', value: '轮胎险' },
      { name: 'acc', value: '事故' },
      { name: 'crash', value: '刮蹭' },
  ...
  saveDataToServer: function(imageOk) {
    ...
    const carList = db.collection('carList')
    db.collection('carList').add({
      data: {
        ...
        year: that.data.year,
        km: that.data.km,
        restKm: that.data.restKm,
        mPay: that.data.monthlyPay,
        dpBack: that.data.dpBack,
        dp: that.data.dp,
        images: imageOk,
        mode:that.data.mode,
